...
export type ReopenStaleClaimsOptions = {
...
export type ReopenStaleClaimsResult = {
...
export function isClaimFresh(
  ...
  return Boolean(claimedAt) && !claimLooksStale(claimedAt, now);
}
 ...
 */
export async function reopenStaleClaims(
  options: ReopenStaleClaimsOptions = {},
): Promise<ReopenStaleClaimsResult> {
  ...
  return { reopenedCount: result.count, cutoff };
}
 ...
 */
export async function reopenStaleClaimById(
  itemId: string,
